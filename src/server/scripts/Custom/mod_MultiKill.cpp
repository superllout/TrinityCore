...
void LoadDataFromDataBase()
{
    ...
    TC_LOG_INFO("server.loading", "Loading MultiKill...");
    ...
    if (!result)
    {
        TC_LOG_INFO("server.loading", ">> `world_multikill` is empty");
        return;
    }
    ...
    do
    {
        ...
        {
            TC_LOG_ERROR("sql.sql", "MultiKill: KillType (Type: %u) non-existing", MultiKillTemp.Type);
            continue;
        }

        if (MultiKillTemp.Type == KILL_TYPE_FIRST && MultiKillTemp.Count != 0)
        {
            TC_LOG_ERROR("sql.sql", "MultiKill: FirstKill (Type: %u) need count = 0", MultiKillTemp.Type);
            continue;
        }

        if (MultiKillTemp.BuffFirst != 0 && !sSpellMgr->GetSpellInfo(MultiKillTemp.BuffFirst))
        {
            TC_LOG_ERROR("sql.sql", "MultiKill: BuffFirst spell (ID: %u) non-existing", MultiKillTemp.BuffFirst);
            MultiKillTemp.BuffFirst = 0;
        }

        if (MultiKillTemp.BuffSecond != 0 && !sSpellMgr->GetSpellInfo(MultiKillTemp.BuffSecond))
        {
            TC_LOG_ERROR("sql.sql", "MultiKill: BuffSecond spell (ID: %u) non-existing", MultiKillTemp.BuffSecond);
    ...
    TC_LOG_INFO("server.loading", ">> Loaded %u count for MultiKill in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
