...
#define GOSSIP_SQUIRE_ITEM_3 "Ask Cavin to summon the Black Knight"
...
public:
    ...
    {
        player->PlayerTalkClass->ClearMenus();
        if (uiAction == GOSSIP_ACTION_INFO_DEF+1 || uiAction == GOSSIP_ACTION_INFO_DEF+3)
        {
            player->CLOSE_GOSSIP_MENU();
            if (creature->GetEntry() == NPC_SQUIRE_DAVID)
...
public:
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 /*uiSender*/, uint32 uiAction)
    {
        ...
        if (uiAction == GOSSIP_ACTION_INFO_DEF+1)
        {
            ...
            creature->GetAI()->DoAction(EVENT_STARTr);
        }
        return true;
    }
